...
typedef IEvent<void(std::shared_ptr<GameObject>, const Vector2i&)> TryToCreateGameObjectEvent;
typedef IEvent<void(std::shared_ptr<GameObject>)> RemoveGameObjectEvent;

#define TRY_CREATE_GAME_OBJECT(callbackFunction) std::function<void(std::shared_ptr<GameObject>, const Vector2i&)>(std::bind(callbackFunction, this, std::placeholders::_1,std::placeholders::_2))
...
public:
	...
	GameObject* CheckForGameObjectOnTile(const Vector2i& aGridIndexToCheck);

	void AddToGrid(std::shared_ptr<GameObject> aObjectToAdd, Vector2i aGridIndex);
	void RemoveFromGrid(Vector2i aGridIndex);
